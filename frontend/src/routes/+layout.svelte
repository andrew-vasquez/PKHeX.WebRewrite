			...
			<header class="sticky top-0 z-30 border-b border-[var(--pkh-shell-border)] bg-[var(--pkh-shell-panel)] lg:hidden">
				<div class="flex items-center justify-between gap-3 px-4 py-3">
					...
					<div class="flex items-center gap-3">
						...
						<Button variant="default" onclick={() => (mobileNavOpen = !mobileNavOpen)}>Menu</Button>
					</div>
				</div>

				{#if mobileNavOpen}
					<nav
						class="space-y-2 overflow-hidden border-t border-[var(--pkh-shell-border)] px-4 py-4"
						...
						{#each navItems as item (item.href)}
							<a
								...
								class={[
									...
									page.url.pathname === item.href
										? 'bg-[color:var(--pkh-shell-primary)]/12 text-[var(--pkh-shell-primary)] dark:bg-[color:var(--pkh-shell-primary)]/18 dark:text-[#c7d8ff]'
